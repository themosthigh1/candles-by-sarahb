    ...
    <div className={classNames.container}>
      {images.map((image, index) => (
        <Image
          ...
          height={900}
          alt={`Slide ${index + 1}`}
          className={`w-full h-full mt-14 object-contain ${
...
const classNames = {
  ...
  previousButton:
    "absolute flex h-14 w-14 top-1/2 left-2 transform -translate-y-1/2 px-4 py-2 bg-black text-white rounded-full",
  nextButton:
    "absolute flex h-14 w-14 top-1/2 right-2 transform -translate-y-1/2 px-4 py-2 bg-black text-white rounded-full",
};
